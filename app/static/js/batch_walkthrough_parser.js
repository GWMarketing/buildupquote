...
(function () {
  ...
  function parseWalkthroughTranscript(transcript) {
    ...
    // Segment on the NORMALIZED text so spoken numbers read as digits and the
    // qty+unit anchors split line items away from address/title/client phrases.
    ...
    for (var i = 0; i < segments.length; i++) {
      ...
      if (isLineItemPhrase(segNorm)) {
        ...
        else data.line_items.push(item);
        continue;
      }
    ...
    // Global assembly sweep: captures every "drywall 12 by 14 9 ft" even when
    // a later assembly has spoken dims and didn't split into its own segment.
